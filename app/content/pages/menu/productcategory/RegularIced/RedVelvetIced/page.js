...
const RedVelvetIced = () => {
  ...
  React.useEffect(() => {
    ...
    async function getDataFromFirestore() {
      ...
      const matchingObject = data.find(obj => obj.name === user?.user_metadata.full_name && obj.menu_name === "Red Velvet Iced");
      if (matchingObject) {
  ...
  return (
    ...
    >
      <PathAndBackButton>
        <Path>
          ...
          <Slash>/</Slash>
          <ItemText>{props.items[1].list[6].name}</ItemText>
        </Path>
        <Link href="/content/pages/menu/productcategory/RegularIced">
      ...
      <ProductHero $background={props.items[1].background}>
        <Img src={`/images/${props.items[1].list[6].image}`} alt={props.items[1].list[6].name} width={500} height={500}/>
        <ProductDesc>
          <ItemName>{props.items[1].list[6].name}</ItemName>
          <RatingWrap>
            <StarsReview>
              <Rating>
                {props.items[1].list[6].stars}★
              </Rating>
              <TextReview>
                {props.items[1].list[6].reviews}&nbsp;reviews
              </TextReview>
            </StarsReview>
            {authReady && (
              <h2
          ...
          <DetailDesc>{props.items[1].list[6].description}</DetailDesc>
          <Calory>{props.items[1].list[6].calories}</Calory>
        </ProductDesc>
      </ProductHero>
      <ProductInfoOrder>
        <SizeAndOrder>
    ...
    {rateMenu && (
      <RatingStar>
        ...
        <RatingMenuWrap>
          <RatingImg src={`/images/ReguIced_RedV.jpg`} alt={props.items[1].list[6].name} width={50} height={50}/>
          <RatingItemName>{props.items[1].list[6].name}</RatingItemName>
        </RatingMenuWrap>
        <YellowStars>
          {stars.map((_, index) => {
            return (
              <StarIcon
                ...
                onClick={() => {
                  ...
                  setMenuPic('ReguIced_RedV.jpg');
                  setMenuCategory(props.items[1].name);
                  setMenuName(props.items[1].list[6].name);
                }}
