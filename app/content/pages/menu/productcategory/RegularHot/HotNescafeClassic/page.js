...
const HotNescafe = () => {
  ...
  React.useEffect(() => {
    ...
    .then(data => {
      const matchingObject = data.find(obj => obj.name === user?.user_metadata.full_name && obj.menu_name === "Hot Nescafe Classic");
      if (matchingObject) {
  ...
  return (
    ...
    >
      <PathAndBackButton>
        <Path>
          ...
          <Slash>/</Slash>
          <ItemText>{props.items[2].list[8].name}</ItemText>
        </Path>
        <Link href="/content/pages/menu/productcategory/RegularHot">
      ...
      <ProductHero $background={props.items[2].background}>
        <Img src={`/images/${props.items[2].list[8].image}`} alt={props.items[2].list[8].name} width={500} height={500}/>
        <ProductDesc>
          <ItemName>{props.items[2].list[8].name}</ItemName>
          <RatingWrap>
            <StarsReview>
              <Rating>
                {props.items[2].list[8].stars}★
              </Rating>
              <TextReview>
                {props.items[2].list[8].reviews}&nbsp;reviews
              </TextReview>
            </StarsReview>
            {authReady && (
              <h2
          ...
          <DetailDesc>{props.items[2].list[8].description}</DetailDesc>
          <Calory>{props.items[2].list[8].calories}</Calory>
        </ProductDesc>
      </ProductHero>
      <ProductInfoOrder>
        <SizeAndOrder>
    ...
    {rateMenu && (
      <RatingStar>
        ...
        <RatingMenuWrap>
          <RatingImg src={`/images/ReguHot_Nescafe.jpg`} alt={props.items[2].list[8].name} width={50} height={50}/>
          <RatingItemName>{props.items[2].list[8].name}</RatingItemName>
        </RatingMenuWrap>
        <YellowStars>
          {stars.map((_, index) => {
            return (
              <StarIcon
                ...
                onClick={() => {
                  ...
                  setMenuPic('ReguHot_Nescafe.jpg');
                  setMenuCategory(props.items[2].name);
                  setMenuName(props.items[2].list[8].name);
                }}
