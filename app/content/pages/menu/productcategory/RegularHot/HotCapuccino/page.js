...
const HotCapuccino = () => {
  ...
  React.useEffect(() => {
    ...
    .then(data => {
      const matchingObject = data.find(obj => obj.name === user?.user_metadata.full_name && obj.menu_name === "Hot Capuccino");
      if (matchingObject) {
  ...
  return (
    ...
    >
      <PathAndBackButton>
        <Path>
          ...
          <Slash>/</Slash>
          <ItemText>{props.items[2].list[1].name}</ItemText>
        </Path>
        <Link href="/content/pages/menu/productcategory/RegularHot">
      ...
      <ProductHero $background={props.items[2].background}>
        <Img src={`/images/${props.items[2].list[1].image}`} alt={props.items[2].list[1].name} width={500} height={500}/>
        <ProductDesc>
          <ItemName>{props.items[2].list[1].name}</ItemName>
          <RatingWrap>
            <StarsReview>
              <Rating>
                {props.items[2].list[1].stars}★
              </Rating>
              <TextReview>
                {props.items[2].list[1].reviews}&nbsp;reviews
              </TextReview>
            </StarsReview>
            {authReady && (
              <h2
          ...
          <DetailDesc>{props.items[2].list[1].description}</DetailDesc>
          <Calory>{props.items[2].list[1].calories}</Calory>
        </ProductDesc>
      </ProductHero>
      <ProductInfoOrder>
        <SizeAndOrder>
    ...
    {rateMenu && (
      <RatingStar>
        ...
        <RatingMenuWrap>
          <RatingImg src={`/images/ReguHot_Capu.jpg`} alt={props.items[2].list[1].name} width={50} height={50}/>
          <RatingItemName>{props.items[2].list[1].name}</RatingItemName>
        </RatingMenuWrap>
        <YellowStars>
          {stars.map((_, index) => {
            return (
              <StarIcon
                ...
                onClick={() => {
                  ...
                  setMenuPic('ReguHot_Capu.jpg');
                  setMenuCategory(props.items[2].name);
                  setMenuName(props.items[2].list[1].name);
                }}
